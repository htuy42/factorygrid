...
func makeTile(waitForSync bool, outChan chan *tile, x, y, size int, msBetweenTicks int64, entitiesOut chan *entity) *tile {
	res := tile{}
...
func (t *tile) runInteraction(interaction *proto.Interaction) {
	...
	localTile := &t.internalContents[interaction.Y][interaction.X]
	...
	localTile.dirty = true
	t.dirty = true
	if interaction.InteractionChar == "1" {
		localTile.typeId = 1
	} else if interaction.InteractionChar == "0" {
		localTile.typeId = 0
	} else if interaction.InteractionChar == "2" {
		localTile.typeId = 2
	} else if interaction.InteractionChar == "b" {
		newEnt := makeEntity(interaction.X+int32(t.x)*t.size32, interaction.Y+int32(t.y)*t.size32, BEETLE)
		t.addEntity(newEnt)
	} else {
		localTile.dirty = false
		t.dirty = false
	}
}
